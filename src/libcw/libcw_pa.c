...
bool cw_is_pa_possible(const char *device)
{
	...
	if (!cw_dlopen_internal(library_name, &cw_pa.handle)) {
		cw_debug_msg (&cw_debug_object, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_ERROR,
			      "libcw/pa: is possible: can't access PulseAudio library \"%s\"", library_name);
		return false;
	}
	...
	if (rv < 0) {
		cw_debug_msg (&cw_debug_object, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_ERROR,
			      "libcw/pa: is possible: failed to resolve PulseAudio symbol #%d, can't correctly load PulseAudio library", rv);
	...
	if (!s) {
		cw_debug_msg (&cw_debug_object, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_ERROR,
			      "libcw/pa: is possible: can't connect to PulseAudio server: %s", cw_pa.pa_strerror(error));
		if (cw_pa.handle) {
			dlclose(cw_pa.handle);
		}
		return false;
	} else {
		/* We do dlclose(cw_pa.handle) in cw_pa_close_device_internal(). */
...
int cw_pa_write_internal(cw_gen_t * gen)
{
	...
	if (rv < 0) {
		cw_debug_msg (&cw_debug_object, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_ERROR,
			      "libcw/pa: write: pa_simple_write() failed: %s", cw_pa.pa_strerror(error));
		return CW_FAILURE;
	} else {
		//cw_debug_msg (&cw_debug_object_dev, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_INFO, "libcw/pa: written %d samples with PulseAudio", gen->buffer_n_samples);
		return CW_SUCCESS;
	}
}
...
pa_simple * cw_pa_simple_new_internal(pa_sample_spec * ss, pa_buffer_attr * ba, const char * device, const char * stream_name, int * error)
{
	...
	pa_simple * s = cw_pa.pa_simple_new(NULL,                  /* Server name (NULL for default). */
					    "libcw2",              /* Descriptive name of client (application name etc.). */
...
int cw_pa_open_device_internal(cw_gen_t * gen)
{
	...
	gen->pa_data.s = cw_pa_simple_new_internal(&gen->pa_data.ss, &gen->pa_data.ba,
 	...
 	if (!gen->pa_data.s) {
		cw_debug_msg (&cw_debug_object_dev, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_ERROR,
			      "libcw/pa: open device: can't connect to PulseAudio server: %s", cw_pa.pa_strerror(error));
		return false;
	}
	...
	if ((gen->pa_data.latency_usecs = cw_pa.pa_simple_get_latency(gen->pa_data.s, &error)) == (pa_usec_t) -1) {
		cw_debug_msg (&cw_debug_object_dev, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_ERROR,
			      "libcw/pa: open device: pa_simple_get_latency() failed: %s", cw_pa.pa_strerror(error));
	}
...
void cw_pa_close_device_internal(cw_gen_t * gen)
{
	if (gen->pa_data.s) {
		...
		if (cw_pa.pa_simple_drain(gen->pa_data.s, &error) < 0) {
			cw_debug_msg (&cw_debug_object_dev, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_ERROR,
				      "libcw/pa: close device: pa_simple_drain() failed: %s", cw_pa.pa_strerror(error));
		}
		cw_pa.pa_simple_free(gen->pa_data.s);
		gen->pa_data.s = NULL;
	} else {
		cw_debug_msg (&cw_debug_object_dev, CW_DEBUG_SOUND_SYSTEM, CW_DEBUG_WARNING,
			      "libcw/pa: close device: called the function for NULL PA sink");
	}
